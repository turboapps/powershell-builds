...
# Not sure why but the page needs to be loaded twice as it fails to load on the first attempt
$Page = curl 'https://www.enterprisedb.com/downloads/postgres-postgresql-downloads' -UseBasicParsing
...
$InstalledVersion = ($VersionLink -split "/")[-2]
...
$ProcessExitCode = RunProcess "$DownloadPath\$InstallerName" "--unattendedmodeui none --mode unattended --superpassword postgres --locale en --prefix c:\pgsql --datadir C:\pg-data" $True
