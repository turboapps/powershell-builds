...
$Vendor = "Adobe"
$AppDesc = "Adobe InCopy is a professional word processor made by Adobe Inc. that integrates with Adobe InDesign."
$AppName = "InCopy"
...
$turboArgs = "try sikulixide --using=oracle/jre-x64 --offline --disable=spawnvm --isolate=merge-user --startup-file=java -- -jar @SYSDRIVE@\SikulixIDE\sikulixide-2.0.5.jar -r $sikulixPath\build.sikuli -f $env:userprofile\desktop\build-sikulix-log.txt"
$ProcessExitCode = RunProcess "turbo.exe" $turboArgs $True
...
Expand-Archive -Path $DownloadPath\InCopy_x64_en_US_WIN_64.zip -DestinationPath $DownloadPath
...
$ProcessExitCode = RunProcess "$DownloadPath\InCopy_x64\Build\Setup.exe" "--silent" $True
CheckForError "Checking process exit code:" 0 $ProcessExitCode $True # Fail on install error
...
# Launch InCopy twice - this will speed up first launch of a new session
# Look for the incopy.exe
$exePath = Get-ChildItem -Path "C:\Program Files\Adobe" -Filter "incopy.exe" -Recurse -ErrorAction SilentlyContinue | Select-Object -ExpandProperty FullName
# Launch the target application
Start-Process -FilePath $exePath
Start-Sleep -Seconds 60
taskkill.exe /f /im incopy.exe /t
Start-Process -FilePath $exePath
Start-Sleep -Seconds 60
taskkill.exe /f /im incopy.exe /t
