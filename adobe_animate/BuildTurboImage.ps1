...
$Vendor = "Adobe"
$AppDesc = "Animate (formerly Flash Professional) lets you create high-quality vector graphics that are scalable, reusable, and adaptable."
$AppName = "Animate"
...
Expand-Archive -Path $DownloadPath\Animate_x64_en_US_WIN_64.zip -DestinationPath $DownloadPath
...
$ProcessExitCode = RunProcess "$DownloadPath\Animate_x64\Build\Setup.exe" "--silent" $True
CheckForError "Checking process exit code:" 0 $ProcessExitCode $True # Fail on install error
...
$InstalledVersion = GetVersionFromRegistry "Adobe Animate"
