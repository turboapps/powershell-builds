...
# The capture runs on 64-bit Windows so Turbo Studio marks the project x64. This is the 32-bit
# Apache build, so retarget the image to x86 - that is what lets it run on 32-bit hosts as well.
$VirtualizationSettings = $xappl.Configuration.SelectSingleNode("VirtualizationSettings")
if ($VirtualizationSettings) {
    $VirtualizationSettings.SetAttribute("targetArchitecture", "x86")
}
