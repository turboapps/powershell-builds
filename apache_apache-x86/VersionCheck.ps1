...
# Read the version out of the win32 .zip name. The arch token case is inconsistent on the page
# (Win64 vs win32) and the VS number changes with each toolchain bump, so match case-insensitively.
$ALMatch = [regex]::Match($Page.Content, "href=`"/download/VS\d+/binaries/httpd-(?<ver>\d+(?:\.\d+)+)-\d+-win32-VS\d+\.zip`"", 'IgnoreCase')
if (-not $ALMatch.Success) {
    WriteLog "Could not find a win32 httpd .zip link on the Apache Lounge download page. Exiting."
