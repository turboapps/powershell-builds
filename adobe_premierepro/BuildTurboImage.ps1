...
$Vendor = "Adobe"
$AppDesc = "Adobe Premiere Pro is a timeline-based and non-linear video editing software application."
...
$ProcessExitCode = RunProcess "$SupportFiles\7za.exe" "x $DownloadPath\CreativeCloudDesktop_x64_en_US_WIN_64.zip -aoa -o$DownloadPath" $True
CheckForError "Checking process exit code:" 0 $ProcessExitCode $True # Fail on install error
$ProcessExitCode = RunProcess "$SupportFiles\7za.exe" "x $DownloadPath\PremierePro_x64_en_US_WIN_64.zip -aoa -o$DownloadPath" $True
CheckForError "Checking process exit code:" 0 $ProcessExitCode $True # Fail on install error
...
$ProcessExitCode = RunProcess "$DownloadPath\PremierePro_x64\Build\Setup.exe" "--silent" $True
CheckForError "Checking process exit code:" 0 $ProcessExitCode $True # Fail on install error
