...
$Vendor = "Adobe"
$AppDesc = "Adobe Prelude video logging and ingest software."
$AppName = "Prelude"
...
Expand-Archive -Path $DownloadPath\Prelude_x64_en_US_WIN_64.zip -DestinationPath $DownloadPath
...
$ProcessExitCode = RunProcess "$DownloadPath\Prelude_x64\Build\Setup.exe" "--silent" $True
CheckForError "Checking process exit code:" 0 $ProcessExitCode $True # Fail on install error
...
$InstalledVersion = GetVersionFromRegistry "Adobe Prelude"

# Get the Year version if it exists from the shortcut name
    $yearVersion = $null
    
    # Get the name of the shortcut without extension
    $shortcut = Get-ChildItem -Path "$env:ALLUSERSPROFILE\Microsoft\Windows\Start Menu\Programs" -Filter "*Prelude*.lnk" | Select-Object -First 1
