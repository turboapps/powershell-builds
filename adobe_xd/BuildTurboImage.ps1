...
$Vendor = "Adobe"
$AppDesc = "Adobe XD is a vector design tool for web and mobile applications."
$AppName = "XD"
...
Expand-Archive -Path $DownloadPath\XD_x64_en_US_WIN_64.zip -DestinationPath $DownloadPath
...
$ProcessExitCode = RunProcess "$DownloadPath\XD_x64\Build\Setup.exe" "--silent" $True
CheckForError "Checking process exit code:" 0 $ProcessExitCode $True # Fail on install error
...
# Get the version from the subfolder containing "Adobe.XD" in the name
$subfolder = Get-ChildItem -Path "C:\Program Files\WindowsApps" -Directory | Where-Object { $_.Name -like "*Adobe.XD*" }
$InstalledVersion = ($subfolder.Name -split "_")[1]
